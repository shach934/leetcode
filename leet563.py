...
class Solution(object):
    def findTilt(self, root):
        ...
        if root is None:
            return 0
        totalTile = [0]
        self.helper(root, totalTile)
        return totalTile[0]
    
    def helper(self, root, totalTile):
        if root is None:
            return 0
        if root.left is None and root.right is None:
            return root.val
        left = self.helper(root.left, totalTile)
        right = self.helper(root.right, totalTile)
        totalTile[0] += abs(left- right)
